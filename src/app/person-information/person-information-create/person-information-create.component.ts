...
@Component({
  ...
  templateUrl: './person-information-create.component.html',
  styleUrls: ['../person-information.css'],
  providers: [
    { provide: MAT_DATE_FORMATS, useValue: MY_DATE_FORMATS }
  ]
})
export class PersonInformationCreateComponent {
  //Initial object
  ...
  constructor(private fb: FormBuilder, private agendaDataService: AgendaDataService, private router: Router) {
    this.person = {
      id: 0,
      firstName: "",
      lastName: "",
  ...
  onSubmit() {
    ...
    if (this.checkFormValidity()) {
      ...
      //Save new person
      const returnedPerson = this.agendaDataService.createPerson(this.person)

      //Make initial object equal to the current object in order to navigate
      this.initialPerson = JSON.stringify(this.person)

      //Navigate to details page
      this.router.navigate(['/persons/' + returnedPerson.id])
    }
  }
